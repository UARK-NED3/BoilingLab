...
| Case | Nominal power (W) | Mean pressure (kPa) | Mean liquid temperature (°C) | `T_sat` (°C) | Maximum `q''` (W/cm^2) | Maximum `T_w` (°C) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
| --- | ---: | --- | --- | --- | --- | --- |
| A | 150 | no | no | reference only | not assigned | lower-power reference |
| B | 180 | yes | no sustained post-transition oscillation | reference only | not assigned | lower-power reference |
| C | 230 | yes; `t_DNB = 150.8 s` | yes; `t_osc = 321.6 s` | 0.050 Hz hydrophone; 0.055 Hz AE | high-activity frame at 72% hydrophone-power percentile | developed oscillatory MEB-like case |
| D | 250 | yes; `t_DNB = 124.8 s` | yes; `t_osc = 308.0 s` | 0.080 Hz hydrophone; 0.080 Hz AE | high-activity frame at 89% hydrophone-power percentile | developed oscillatory MEB-like case |
...
The high-speed video provides a visual basis for interpreting the low-frequency acoustic and thermal oscillations. The oscillation is not a uniform emission of small bubbles. Instead, the developed high-power cases show intervals with fewer visible microbubbles followed by intervals with dense microbubble release above the heated surface. Figure 8 shows representative frames selected from the MP4 videos using a frame-screening metric based on visual plume activity and annotated with the corresponding hydrophone-power percentile. The metric is computed in a central plume region of interest as a weighted sum of edge density, grayscale texture, and dark-pixel fraction; it is intended only to rank frames for reproducible selection. In Case C, the lower-activity representative frame has a visual metric of 0.189 and coincides with the 37th percentile of hydrophone band power, while the higher-activity representative frame has a visual metric of 0.195 and coincides with the 72nd percentile. In Case D, the corresponding visual metrics are 0.187 at the 55th hydrophone-power percentile and 0.192 at the 89th percentile. These small metric differences are not interpreted as bubble-volume measurements. They support the frame selection and show that the many-bubble examples are tied to elevated acoustic-power states. The frames therefore support, rather than prove by themselves, a storage-and-release interpretation: vapor accumulates or reorganizes near the wall, the near-wall vapor/liquid system reaches a release condition, and a burst of microbubbles is emitted into the subcooled liquid.
...
**Figure 7. Literature comparison from the source-status-labeled MEB compilation.** Panel (a) compares heat-flux and wall-superheat points where available. Panel (b) compares MEB signature metrics, including heat-flux scale and frequency scale, with clear symbols for frequency type. The literature compilation is treated as a screening dataset, and reported-text values require figure-level verification before final cross-study scaling.
